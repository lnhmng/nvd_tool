...
IF v_SNCNT=0 THEN
   RAISE e_NO_SN;
END IF;
-------------------BIOS Control --Added by Anthony on 2005-04-19----------------------
if LENGTH(BIOS)>1 then   ----Because not all the MB will upload BIOS to our SFC
   select BIOS_VERSION
   into Standard_BIOS
   ...
   if SN_COUNT=0 then
	  ...
	  values(p_SERIALNUMBER,'',BIOS,'','','',sysdate,'');
	  commit;
   ...
   if SN_COUNT>0 then
      ...
      if (p_FIRST_BIOS is not null) then
          update sfism4.R_MB_BIOSNIC_T
...
FROM SFISM4.R_WIP_TRACKING_T
WHERE SERIAL_NUMBER=p_SERIALNUMBER;
--!Oq2A?@-O|r2A?X_!Lu?T|r2A!?PA_O!?_?XICT?
...
IF p_RESULT='F' THEN
INSERT INTO SFISM4.R_TEST_RESULT_T(SERIAL_NUMBER,STATION_ID,TEST_DATE,TEST_TIME,RESULT,ERROR_CODE,MODEL_NAME,STATION_TYPE,WORK_STATION,OPERATOR,FAILDESC,
MO_NUMBER,FIXTURE_ID)
VALUES(P_SERIALNUMBER,p_STATION_ID,p_WORKDATE,p_WORKTIME,p_RESULT,p_ERROR_CODE,p_MODEL,p_STATION_TYPE,p_WORK_STATION,p_OPERATORID,p_FAILDESC,
p_MO,p_FIXTUREID);
COMMIT;
END IF;
...
BEGIN SFIS1.STN_REC_Z(p_LINE,p_SECTION,p_GROUP,p_STATION,p_MO,p_SERIALNUMBER,p_WORKDATE,p_WORKSECT,'0');
...
SFIS1.UPDATE_RLSA_H(p_SERIALNUMBER,p_LINE,p_GROUP,p_MO,p_ERROR_CODE,RES);
			 --ADD THIS LINE FOR ALARM SYSTEM Anthony Zhang 20040810
RES := 'OK';
    END;
END IF;
ELSIF p_RESULT='F' THEN
...
SFIS1.UPDATE_RLSA_H(p_SERIALNUMBER,p_LINE,p_GROUP,p_MO,p_ERROR_CODE,RES);
			 --ADD THIS LINE FOR ALARM SYSTEM Anthony Zhang 20040810	COMMIT;
END IF;
RES:='OK';
END;
END IF;
...
CREATE DATE: 11 15,2003
UPDATE BY: SHUI CAI LI
UPDATE DATE: 12,18,2003
UPDATE BY: SHUI CAI LI
UPDATE DATE: 04,15,2004
UPDATE BY: ZHANG WEI
UPDATE DATE: 06,04,2004
FOR: ICT TEST DATA
UPDATE BY: ZHANG WEI
UPDATE DATE: 08,10,2004
FOR: ISSD ALARM SYSTEM
RES:
0- OK
1- REPAIR
******************************************************** */
PROCEDURE                               Ipqc_Result_Log_TB_V1(
	   EMP IN VARCHAR2,
	   LINE IN VARCHAR2,
	   SECTION IN VARCHAR2,
	   MYGROUP IN VARCHAR2,
	   ...
	   IF MYGROUP='S_VI' THEN
	           ...
	           IF C_NUM>0 THEN
		           --SELECT GROUP_NEXT INTO C_GROUP_NEXT FROM SFIS1.C_ROUTE_CONTROL_T WHERE GROUP_NAME=C_GROUP AND GROUP_NEXT like'%S_VI%' AND ROUTE_CODE=C_ROUTE ;
			        SELECT COUNT(*) INTO C_NUM FROM SFIS1.C_ROUTE_CONTROL_T WHERE GROUP_NAME=C_GROUP AND GROUP_NEXT like'%S_VI%' AND ROUTE_CODE=C_ROUTE AND GROUP_NEXT NOT LIKE 'R%' ;
		            IF C_NUM<>0 THEN
		               SELECT GROUP_NEXT INTO C_GROUP_NEXT FROM SFIS1.C_ROUTE_CONTROL_T WHERE GROUP_NAME=C_GROUP AND GROUP_NEXT like'%S_VI%' AND ROUTE_CODE=C_ROUTE AND GROUP_NEXT NOT LIKE 'R%' ;
...
ELSE
       if substr(MYGROUP,1,1)='S' then
		  SFIS1.Multi_Sn_Update(EMP,LINE,SECTION,V_GROUP,DATETIME,EC,UPPER(DATA),MO_DATE,
...
-- update by liuyunjiang 2006-7-18.
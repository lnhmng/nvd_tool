...
BEGIN
   ...
   BEGIN
      ...
      SELECT TO_CHAR (TO_DATE (vr_value, 'YYYY/MM/DD')+1, 'YYYYMMDD')
        INTO v_start_date
        FROM sfis1.C_PARAMETER_INI
       WHERE     PRG_NAME = 'O_REPORT'
             AND vr_class = 'NVD'
   ...
   END;
    ...
    WHERE     PRG_NAME = 'O_REPORT'
          AND vr_class = 'NVD'
          AND VR_ITEM = 'O_GET_REPORT_DATA_T'
          AND vr_name = 'LAST_GET_TIME_FOR_8AM';
   ...
   v_start_time := TO_CHAR (TO_DATE (v_start_date, 'YYYY/MM/DD'), 'YYYY/MM/DD') || ' 08:00:00';
   v_end_time := v_end_date || ' 14:30:00';
   ...
   AND (cc.serial_number IS NULL or aa.in_station_time<cc.test_time);
      ...
      SELECT DISTINCT aa.model_name,
            ...
            WHERE aa.test_time = bb.test_time
             -- add by    20200311  luo yang and refloat by lsc why luo yang  

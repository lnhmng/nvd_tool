PROCEDURE               iTest_Dino_Fbt (
p_SERIALNUMBER 	  IN VARCHAR2,
p_STATION_ID 	  IN VARCHAR2,---Now we use it to pass the Machine_code
p_TEST_DATE 	  IN VARCHAR2,
p_TEST_TIME 	  IN VARCHAR2,
p_RESULT 	  	  IN VARCHAR2,
p_ERROR_CODE 	  IN VARCHAR2,
p_STATION_TYPE    IN VARCHAR2,
p_WORK_STATION    IN NUMBER,----------not used
...
-- CHECK IF THE CURRENT STATION EXISTING
	...
	WHERE STATION_CODE=p_STATION_ID;
...
IF p_CALLRES<>'OK' THEN
   IF p_FAILDESC<>'DIE' THEN
      IF (p_CALLRES='GOTO-R_FBT') or (p_CALLRES='GO-R_FBT') THEN
         IF (p_FAILQTY=1 AND p_PASSQTY=0) OR (p_FAILQTY=1 AND p_PASSQTY=1)THEN
             IF p_RESULT='P' THEN
...
INSERT INTO SFISM4.R_TEST_RESULT_T(SERIAL_NUMBER,STATION_ID,TEST_DATE,TEST_TIME,RESULT,ERROR_CODE,MODEL_NAME,STATION_TYPE,WORK_STATION,OPERATOR, FAILDESC,MO_NUMBER)
VALUES (P_SERIALNUMBER,31,p_WORKDATE,p_WORKTIME,p_RESULT,p_ERROR_CODE,p_MODEL,p_STATION_TYPE,p_WORK_STATION,p_OPERATORID,p_FAILDESC,p_MO);
COMMIT;
END IF;
...
IF p_RESULT='P' THEN
   ...
   IF (p_PASSQTY=1 AND p_FAILQTY=0) OR (p_PASSQTY=2 AND p_FAILQTY=1) THEN
	  ...
	  SFIS1.UPDATE_RLSA_H(p_SERIALNUMBER,p_LINE,p_GROUP,p_MO,p_TEMP_EC,RES);
   END IF;
   RES := 'OK';
   END;
ELSIF p_RESULT='F' THEN
...
IF RES='OK' THEN
   ...
   ELSIF (p_PASSQTY=0 AND p_FAILQTY=1) OR (p_PASSQTY=1 AND p_FAILQTY=1)THEN
      IF p_FAILDESC='DIE' THEN
	     RES:='SCAN OK';
      ELSE
   	     RES:='2';
   ...
   ELSIF (p_PASSQTY=0 OR p_PASSQTY=1)AND (p_FAILQTY=2 OR p_FAILQTY=3 OR p_FAILQTY=4 ) THEN
      RES:='3';
   END IF;
END IF;
...
FOR: DINO FBT TEST DATA
UPDATE BY: SHUICAI LI
UPDATE DATE:05,06,2004
RES:
0- OK
3- REPAIR
2- RETEST

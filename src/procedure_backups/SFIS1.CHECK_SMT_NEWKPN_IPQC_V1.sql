PROCEDURE       CHECK_SMT_NEWKPN_IPQC_V1(STATION_NUM IN VARCHAR2,MACHINE IN VARCHAR2,
...
-- HHPN OR PKG ID
   IF PID IS NOT NULL AND PID <>'N/A' THEN
    ...
    SELECT COUNT(*) INTO C_COUNT_1
     FROM IQC.R_MSD_DETAIL_T A, IQC.R_KPN_INCOMING_T B
    WHERE A.HHPN = B.HH_PN AND PKG_ID = PID;
    
    IF C_COUNT_1>0 THEN
        SELECT COUNT(*) INTO  C_COUNT_1 FROM IQC.R_MSD_PKG_LOG_T WHERE PKG_ID=PID;
        IF  C_COUNT_1 =0 THEN

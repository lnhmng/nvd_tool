PROCEDURE                               SP_PLMS_AUTOLOGGING_NEW(
   i_user           IN     VARCHAR2,
...
BEGIN             
   ...
   -- modify by Liujiang in 20210220, logging method throw a new condition( i_station_code or i_group_name) == SFIS1.SP_PLMS_AUTOLOGGING_3
    -- modify by Liujiang in 20210410,  --fist fail ,allow to test again
    -- modify by Liujaing on 20210415  ---- 更新rec 報表來源數据
  ---------------------------------------
   ...
   ELSE   
     v_section := i_group_name;
     v_mygroup := i_group_name;
     v_mystation := i_group_name;
   END IF;
   ...
   THEN
      ...
      --INSERT_R102(LINE,SECTION,MYGROUP,MO,MO_DATE,W_SECTION,F_FLAG);  -- 模板
        INSERT_R102(v_current_line,v_section,v_mygroup,v_mo,p_WORKDATE,p_WORKSECT,'0'); -- 更新rec 報表來源數据
   ...
   THEN
       ...
       SELECT COUNT (1)
         INTO v_count
         FROM SFISM4.R_REPAIR_NUMBER
        WHERE serial_number = v_sn and group_name = v_mygroup ;

        IF v_count = 0      --fist fail ,allow to test again
        THEN
            Insert into SFISM4.R_REPAIR_NUMBER(SERIAL_NUMBER, GROUP_NAME, NUM)
             Values (v_sn, v_mygroup, '1');

            o_flag := '-1';
            o_res := 'First fail in '|| v_mygroup ||' allow to test again!';          
            RETURN;

        ELSIF v_count > 0
        THEN
            o_res := 'Update fail number info error';
            UPDATE SFISM4.R_REPAIR_NUMBER
               SET NUM = NUM + 1
             WHERE SERIAL_NUMBER = v_sn ;        
          ...
          INSERT_R102(v_current_line,v_section,v_mygroup,v_mo,p_WORKDATE,p_WORKSECT,'1'); -- 更新rec 報表來源數据
          COMMIT;

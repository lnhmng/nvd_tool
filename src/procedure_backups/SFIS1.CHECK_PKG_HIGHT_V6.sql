PROCEDURE       CHECK_PKG_HIGHT_V6  --added by maggiechang for S000003611 based on SFIS1.CHECK_PKG_HIGHT_V4
(
...
C_COUNT_3                     NUMBER;
C_COUNT_4                     NUMBER;
C_COUNT_PKG                   NUMBER;---ADD BY LLF 2017-09-04
C_OUTPUT                     VARCHAR(64);
...
BEGIN
   ...
   C_FEEDER := FEEDER;
    ...
    --ADD BY LLF 2017-09-04 BEGIN  
    SELECT COUNT(*) INTO C_COUNT_PKG
     FROM IQC.R_MSD_DETAIL_T A, IQC.R_KPN_INCOMING_T B
    WHERE A.HHPN = B.HH_PN AND PKG_ID = PKG;

    IF C_COUNT_PKG>0 THEN
        SELECT COUNT(*) INTO C_COUNT_PKG FROM IQC.R_MSD_PKG_LOG_T WHERE PKG_ID=PKG;
        IF C_COUNT_PKG=0 THEN
           RES:='ERROR:MSD NOT SCAN OPENING';
           RAISE E_ERROR;
        ELSE
           SELECT count(1) INTO C_COUNT_PKG
           FROM SMTINFO.R_SMT_PKGID_LOG_T WHERE PKG_ID=PKG;

           IF C_COUNT_PKG>0 THEN
               SELECT count(1) INTO C_COUNT_PKG FROM IQC.R_MSD_PKG_LOG_T WHERE PKG_ID=PKG AND IN_STATION_TIME>=(SELECT MAX(BEGIN_TIME) 
               FROM SMTINFO.R_SMT_PKGID_LOG_T WHERE PKG_ID=PKG);
                IF C_COUNT_PKG=0 THEN
                   RES:='ERROR:MSD NOT SCAN OPENING';
                   RAISE E_ERROR; 
   ...
   END IF;      
    ...
    SFIS1.CHECK_PKG_MSD_V1(EMP,PKG,LINE,PPN,'N/A',RES);

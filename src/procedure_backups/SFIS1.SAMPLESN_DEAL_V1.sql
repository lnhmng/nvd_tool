...
sample_pnlist1      varchar2(60);
sample_pnlist2      varchar2(60);
sample_pnlist3      varchar2(60);
skpn_qty1      int;
skpn_qty2      int;
skpn_qty3      int;
tmp_flag   varchar2(5);
...
CURSOR getkpnlist(str_pn varchar2, spn_qty int)
IS
  select regexp_substr(str_pn,'[^#]+',1,rownum) as kpn from dual connect by rownum<= 1+spn_qty ;

BEGIN
    -- create base on SFIS1.SAMPLESN_DEAL  for spare key_part_no --liujiang20230919
   RES:='OK';
      exceed_day := 30 ;
   ...
   if TRANTYPE='CHECKSN' THEN
       ...
       if count1>0 then
        ...
        --每個下階料可能存在替代料
          SELECT NVL(PN1||'#'||SPARE_KPN1||'#'||SPARE_KPN2||'#'||SPARE_KPN3,''),a.QTY into sample_pnlist1, sample_qty 
            FROM SFIS1.C_SAMPLESN_BIND_SET a, (select * from SFIS1.C_SAMPLESN_BIND_SPN where BIND_STATION = GROUPNAME ) b
            WHERE a.PARENTPARTNO= modelname and A.PARENTPARTNO =B.PARENTPARTNO(+) and A.PN1=B.KEY_PART_NO(+) ;

          SELECT NVL(PN2||'#'||SPARE_KPN1||'#'||SPARE_KPN2||'#'||SPARE_KPN3,''),a.QTY into sample_pnlist2, sample_qty
            FROM SFIS1.C_SAMPLESN_BIND_SET a, (select * from SFIS1.C_SAMPLESN_BIND_SPN where BIND_STATION = GROUPNAME ) b
            WHERE a.PARENTPARTNO= modelname and A.PARENTPARTNO =B.PARENTPARTNO(+) and A.PN2=B.KEY_PART_NO(+) ;

          SELECT NVL(PN3||'#'||SPARE_KPN1||'#'||SPARE_KPN2||'#'||SPARE_KPN3,''),a.QTY into sample_pnlist3, sample_qty
            FROM SFIS1.C_SAMPLESN_BIND_SET a, (select * from SFIS1.C_SAMPLESN_BIND_SPN where BIND_STATION = GROUPNAME ) b
            WHERE a.PARENTPARTNO= modelname and A.PARENTPARTNO =B.PARENTPARTNO(+) and A.PN3=B.KEY_PART_NO(+) ;                                

          RES:='OK,'|| sample_pnlist1||','||sample_pnlist2||','||sample_pnlist3||','||sample_qty; 
       else
    ...
    if TRANTYPE='BIND' THEN
       ...
       -- SELECT NVL(PN1,''),NVL(PN2,''),NVL(PN3,''),QTY into sample_pn1, sample_pn2, sample_pn3,sample_qty FROM SFIS1.C_SAMPLESN_BIND_SET WHERE PARENTPARTNO= modelname;
        ...
        --每個下階料可能存在替代料
          SELECT NVL(PN1||'#'||SPARE_KPN1||'#'||SPARE_KPN2||'#'||SPARE_KPN3,''),a.QTY ,NVL(B.QTY,0)  into sample_pnlist1, sample_qty,skpn_qty1 
            FROM SFIS1.C_SAMPLESN_BIND_SET a, (select * from SFIS1.C_SAMPLESN_BIND_SPN where BIND_STATION = GROUPNAME ) b
            WHERE a.PARENTPARTNO= modelname and A.PARENTPARTNO =B.PARENTPARTNO(+) and A.PN1=B.KEY_PART_NO(+) ;

          SELECT NVL(PN2||'#'||SPARE_KPN1||'#'||SPARE_KPN2||'#'||SPARE_KPN3,''),a.QTY ,NVL(B.QTY,0) into sample_pnlist2, sample_qty,skpn_qty2 
            FROM SFIS1.C_SAMPLESN_BIND_SET a, (select * from SFIS1.C_SAMPLESN_BIND_SPN where BIND_STATION = GROUPNAME ) b
            WHERE a.PARENTPARTNO= modelname and A.PARENTPARTNO =B.PARENTPARTNO(+) and A.PN2=B.KEY_PART_NO(+) ;

          SELECT NVL(PN3||'#'||SPARE_KPN1||'#'||SPARE_KPN2||'#'||SPARE_KPN3,''),a.QTY ,NVL(B.QTY,0) into sample_pnlist3, sample_qty,skpn_qty3
            FROM SFIS1.C_SAMPLESN_BIND_SET a, (select * from SFIS1.C_SAMPLESN_BIND_SPN where BIND_STATION = GROUPNAME ) b
            WHERE a.PARENTPARTNO= modelname and A.PARENTPARTNO =B.PARENTPARTNO(+) and A.PN3=B.KEY_PART_NO(+) ;         


        if to_number(qty)>=1 then

            tmp_flag:='N';
            FOR rec IN getkpnlist (sample_pnlist1, skpn_qty1)   --判斷 下階SN 屬於哪個替代料之一
            LOOP
                if tmp_flag ='N' then
                    sample_pn1:= rec.kpn;
        ...
        if to_number(qty)>=2 then

            tmp_flag:='N';
            FOR rec IN getkpnlist (sample_pnlist2, skpn_qty2)   --判斷 下階SN 屬於哪個替代料之一
            LOOP
                if tmp_flag='N' then
                    sample_pn2:= rec.kpn;
        ...
        if to_number(qty)>=3 then

            tmp_flag:='N';
            FOR rec IN getkpnlist (sample_pnlist3, skpn_qty3)   --判斷 下階SN 屬於哪個替代料之一
            LOOP
                if tmp_flag='N' then
                    sample_pn3:= rec.kpn;
                    ...
                    if count1>0 then
                        ...
                        RES:='PN2 '||sample_pn3||' TE not set SN RULE,Contact TE';                                    
                    end if;                 
                end if;
        ...
        if qty='1' then
           ...
           END IF; 
            ...
            -- check BIND 900AOI PASS by axie 20230529，曾有綁過的就要之後就要AOI pass才可用 by axie
            /*select count(C_SN) into BIND_cnt from SFISM4.R_SAMPLESN_BIND_DETAIL 
            where C_SN=SAMPLESN1 and trim(SAMPLESN1) like 'E3665%' AND GROUP_NAME like 'BBD%' ;*/
            --if instr(TRANTYPE, 'NOT_CHECK') = 0 and sample_pn1 not like '%HS' and trim(SAMPLESN1) like 'E3665%' /*and BIND_cnt > 0*/ then
            --if trim(SAMPLESN1) like 'E3665%' or trim(SAMPLESN1) like 'E4821%' /*and BIND_cnt > 0*/ then
            if trim(sample_pn1) like 'E3665%' or trim(sample_pn1) like 'E4821%' then
                select count(*) into BIND_cnt2 from SFISM4.R_SAMPLESN_BIND_DETAIL where GROUP_NAME like 'BBD%' and C_SN=SAMPLESN1  ;
                select count(*) into BIND_cnt3 from SFISM4.r_bind_900aoi_t where BIND_SN=SAMPLESN1   ;
                if BIND_cnt2 = 0  then
                    if BIND_cnt3 > 0 then
                        select count(*) into BIND_cnt3 from (
                            select SFISM4.r_bind_900aoi_t.*, ROW_NUMBER() OVER (ORDER BY TEST_DATE desc) r 
                            from SFISM4.r_bind_900aoi_t where BIND_SN=SAMPLESN1 
                        ) where r = 1 and RESULT='P' and sysdate-TEST_DATE <= exceed_day ;
                        if (BIND_cnt3 = 0) then
                            res := 'BIND1 NOT WITHIN LAST TIME AOI DATA，PLEASE GOTO-900AOI TEST AGAIN' ;
                            return ;
                        end if ;                                        
                    end if;                
                else
                    select count(BIND_SN) into BIND_cnt from (
                        select * from (
                            select * from SFISM4.R_SAMPLESN_BIND_DETAIL 
                            where GROUP_NAME like 'BBD%' and C_SN=SAMPLESN1 
                            order by LASTEDITDT desc
                        ) where rownum <=1
                    ) BIND_DETAIL inner join (
                        select * from (
                            select SFISM4.r_bind_900aoi_t.*, ROW_NUMBER() OVER (ORDER BY TEST_DATE desc) r 
                            from SFISM4.r_bind_900aoi_t where BIND_SN=SAMPLESN1 
                        ) where r = 1 and RESULT='P' and sysdate-TEST_DATE <= exceed_day
                    ) bind_900aoi_t on bind_900aoi_t.TEST_DATE > BIND_DETAIL.LASTEDITDT ;
                    if BIND_cnt = 0 then
                        res := 'BIND1 NOT WITHIN LAST TIME AOI DATA，PLEASE GOTO-900AOI TEST AGAIN' ;
                        return ;
                    end if ;
            ...
            -- check BIND 900AOI PASS by axie 20230529 end
        ...
        if qty='2' then
            ...
            -- check BIND 900AOI PASS by axie 20230529，曾有綁過的就要之後就要AOI pass才可用 by axie
            /*select count(C_SN) into BIND_cnt from SFISM4.R_SAMPLESN_BIND_DETAIL 
            where C_SN=SAMPLESN2 and trim(SAMPLESN2) like 'E3665%' AND GROUP_NAME like 'BBD%' ;*/
            --if instr(TRANTYPE, 'NOT_CHECK') = 0 and sample_pn2 not like '%HS' and trim(SAMPLESN2) like 'E3665%' /*and BIND_cnt > 0*/ then
            --if trim(SAMPLESN2) like 'E3665%' or trim(SAMPLESN2) like 'E4821%' /*and BIND_cnt > 0*/ then
            if trim(sample_pn2) like 'E3665%' or trim(sample_pn2) like 'E4821%' then
                select count(*) into BIND_cnt2 from SFISM4.R_SAMPLESN_BIND_DETAIL where GROUP_NAME like 'BBD%' and C_SN=SAMPLESN2  ;
                select count(*) into BIND_cnt3 from SFISM4.r_bind_900aoi_t where BIND_SN=SAMPLESN2   ;
                if BIND_cnt2 = 0  then
                    if BIND_cnt3 > 0 then
                        select count(*) into BIND_cnt3 from (
                            select SFISM4.r_bind_900aoi_t.*, ROW_NUMBER() OVER (ORDER BY TEST_DATE desc) r 
                    ...
                    select count(BIND_SN) into BIND_cnt from (
                        select * from (
                            select * from SFISM4.R_SAMPLESN_BIND_DETAIL 
                            where GROUP_NAME like 'BBD%' and C_SN=SAMPLESN2 
                            order by LASTEDITDT desc
                        ) where rownum <=1
                    ) BIND_DETAIL inner join (
                        select * from (
                            select SFISM4.r_bind_900aoi_t.*, ROW_NUMBER() OVER (ORDER BY TEST_DATE desc) r 
                            from SFISM4.r_bind_900aoi_t where BIND_SN=SAMPLESN2 
                        ) where r = 1 and RESULT='P' and sysdate-TEST_DATE <= exceed_day
                    ) bind_900aoi_t on bind_900aoi_t.TEST_DATE > BIND_DETAIL.LASTEDITDT ;
                    if BIND_cnt = 0 then
                        res := 'BIND1 NOT WITHIN LAST TIME AOI DATA，PLEASE GOTO-900AOI TEST AGAIN' ;
        ...
        if qty='3' then
           ...
           select count(*) into count1 from SFIS1.C_SAMPLESN_REPAIR where sn = SAMPLESN3 and type='NG';
            ...
            -- check BIND 900AOI PASS by axie 20230529，曾有綁過的就要之後就要AOI pass才可用 by axie
            /*select count(C_SN) into BIND_cnt from SFISM4.R_SAMPLESN_BIND_DETAIL 
            where C_SN=SAMPLESN3 and trim(SAMPLESN3) like 'E3665%' AND GROUP_NAME like 'BBD%' ;*/
            --if instr(TRANTYPE, 'NOT_CHECK') = 0 and sample_pn3 not like '%HS' and trim(SAMPLESN3) like 'E3665%' /*and BIND_cnt > 0*/ then
            --if trim(SAMPLESN3) like 'E3665%' or trim(SAMPLESN3) like 'E4821%' /*and BIND_cnt > 0*/ then
            if trim(sample_pn3) like 'E3665%' or trim(sample_pn3) like 'E4821%' then
                select count(*) into BIND_cnt2 from SFISM4.R_SAMPLESN_BIND_DETAIL where GROUP_NAME like 'BBD%' and C_SN=SAMPLESN3  ;
                select count(*) into BIND_cnt3 from SFISM4.r_bind_900aoi_t where BIND_SN=SAMPLESN3   ;
                if BIND_cnt2 = 0  then
                    if BIND_cnt3 > 0 then
                        select count(*) into BIND_cnt3 from (
                            select SFISM4.r_bind_900aoi_t.*, ROW_NUMBER() OVER (ORDER BY TEST_DATE desc) r 
                            from SFISM4.r_bind_900aoi_t where BIND_SN=SAMPLESN3 
                        ) where r = 1 and RESULT='P' and sysdate-TEST_DATE <= exceed_day ;
                        if (BIND_cnt3 = 0) then
                            res := 'BIND1 NOT WITHIN LAST TIME AOI DATA，PLEASE GOTO-900AOI TEST AGAIN' ;
                            return ;
                    ...
                    select count(BIND_SN) into BIND_cnt from (
                        select * from (
                            select * from SFISM4.R_SAMPLESN_BIND_DETAIL 
                            where GROUP_NAME like 'BBD%' and C_SN=SAMPLESN3 
                            order by LASTEDITDT desc
                        ) where rownum <=1
                    ) BIND_DETAIL inner join (
                        select * from (

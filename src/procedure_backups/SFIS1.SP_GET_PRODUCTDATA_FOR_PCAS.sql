PROCEDURE                                                                                     sp_get_productdata_for_pcas
AS
   v_count         NUMBER;
...
BEGIN
   ...
   BEGIN
      SELECT   TRIM (VR_NAME)
       ...
       WHERE       PRG_NAME = 'PCAS_GET_PROD_DATA'
  ...
  -- v_start_time := v_start_date || ' 07:30:00'; --delete by jinglong 20160622
   ...
   IF v_count >= 1
   THEN
      v_res := 'delete data error from table : mes4.mfsysevent_nvd_temp@ODMDB';
      
                       
      DELETE FROM   mes4.mfsysevent_nvd_temp@ODMDB WHERE BU_NAME = 'NVD';
    

      v_res := 'insert into mes4.mfsysevent_nvd_temp@ODMDB error';

      INSERT INTO mes4.mfsysevent_nvd_temp@ODMDB (SYSSERIALNO,
                                                  EVENTNAME,
                                                  SCANDATETIME,
                                                  FACTORYID,
                                                  PRODUCTIONLINE,
         ...
         v_res := 'insert into mes4.mfsysevent_nvd@ODMDB error';
        insert into mes4.mfsysevent_nvd@ODMDB
        ...
        select SYSSERIALNO, EVENTNAME, SCANDATETIME, 
               ...
               WORKORDERNO, ORGSYSSERIALNO, EVENTPASS, 
               EVENTFAIL, WORKTIME, BU_NAME
        from mes4.mfsysevent_nvd_temp@ODMDB a
        where not exists (select 0 from mes4.mfsysevent_nvd@ODMDB b 

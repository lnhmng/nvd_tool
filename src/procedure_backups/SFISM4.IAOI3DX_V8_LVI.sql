PROCEDURE               IAOI3DX_V8_LVI
/*********************************************
                      **
*FOLLOW  PROCEDURE SFISM4.iaoi3dx_v7
Description: To act on the *.3dx file       **
createtor: Ruanshiqiao                        **
Date:2024-12-13                             **
...
BEGIN
   ...
   SELECT count(SERIAL_NUMBER) into sn_link_qty FROM SFISM4.R_PCB_DATECODE_T
    WHERE GROUP_ID IN(SELECT GROUP_ID FROM SFISM4.R_PCB_DATECODE_T WHERE SERIAL_NUMBER=BARCODE);--??????
      IF sn_link_qty>1 THEN
        SELECT SERIAL_NUMBER into barcode2 FROM SFISM4.R_PCB_DATECODE_T --WHERE SERIAL_NUMBER=BARCODE;
         WHERE GROUP_ID IN(SELECT GROUP_ID FROM SFISM4.R_PCB_DATECODE_T WHERE SERIAL_NUMBER=BARCODE) AND SERIAL_NUMBER<>BARCODE;
      END IF;
            ...
            THEN
                ...
                if(sn_link_qty <2) then--??2???????
                   ...
                   INSERT INTO SFISM4.R_AOI_MEMORY_T (SERIAL_NUMBER, LOCATION, BARCODE, GROUP_NAME, IN_STATION_TIME)
                    ...
                    close data_cursor;*/
                        if(LOC LIKE '%_1') then--ADD BY RUANSHIQIAO 20250405 FOR LIANGBAN 
                            insert into SFISM4.AOIMEM_T values(BARCODE,LOC,BSN,sysdate);
                           INSERT INTO SFISM4.R_AOI_MEMORY_T (SERIAL_NUMBER, LOCATION, BARCODE, GROUP_NAME, IN_STATION_TIME)
                                                       VALUES(BARCODE, LOC, BSN, p_group,sysdate);--Add by Ruanshiqiao 20240815 to insert SFISM4.R_AOI_MEMORY_T
                        else
                                insert into SFISM4.AOIMEM_T values(BARCODE2,LOC,BSN,sysdate);
                                INSERT INTO SFISM4.R_AOI_MEMORY_T (SERIAL_NUMBER, LOCATION, BARCODE, GROUP_NAME, IN_STATION_TIME)
                                                       VALUES(BARCODE2, LOC, BSN, p_group,sysdate);--Add by Ruanshiqiao 20240815 to insert SFISM4.R_AOI_MEMORY_T
                        end if;
                end if;
                COMMIT;
           END IF;
...
EXCEPTION
   ...
   WHEN OTHERS
   THEN
      res := 'IAOI3DX_V8_LVI OTHER ERROR' || '\n' || '**END**';
END IAOI3DX_V8_LVI;
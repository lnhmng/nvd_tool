PROCEDURE                                    IAOITXT_V1
/*********************************************
Author : Alex Wang                          **
Date   : 2010-10-26                         **
Description: To act on the *.txt file       **
**********************************************/
(
...
BEGIN
  ...
  IF v_RESULT = 'F' THEN
    EC_TRANSACTION_TXT(TRIM(FAILDESC),
                       EC_CNT,
                       EC_LIST);
    IF EC_CNT = 0 THEN
      RAISE e_EC_ERROR;
...
EXCEPTION
  ...
  WHEN e_CHECK_ERROR THEN
    RES := BARCODE||'>>>'||CHECKRES;

  WHEN e_AOITEST_ERROR THEN
    RES := BARCODE||'>>>'||AOITEST_RES;

  WHEN e_EC_ERROR THEN
    RES := BARCODE||'>>>'||'ERROR CODE ERROR';
  ...
  WHEN OTHERS THEN
    RES := BARCODE||'>>>'||'IAOITXT_V1 OTHER ERROR';

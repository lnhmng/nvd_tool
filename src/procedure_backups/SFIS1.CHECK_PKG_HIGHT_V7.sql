...
IS
   ...
   C_LINE_NAME       VARCHAR2 (25);
BEGIN
  
   C_LOC := LOC;

   SELECT EMP_NO, GROUP_ID
     INTO L_EMPNO, L_STATION_NAME
     FROM SFIS1.C_EMP_DESC_T
    WHERE EMP_NO = EMP AND ROWNUM = 1;

   SELECT STATION_GROUP
     INTO L_STATION_GROUP
     FROM SFIS1.C_PRIV_T
    WHERE GROUP_ID = L_STATION_NAME AND STATION_GROUP = 'SMT';

   --AND STATION_PRIV=FUNC;
   RES := 'OK';
   ...
   THEN
      SELECT COUNT (*)
        INTO C_COUNT_PKG
        FROM iqc.R_MSD_PKGID_LOG_T
       WHERE PKG_ID = PKG AND END_TIME IS NULL;
...
*/
   ...
   IF C_COUNT0 > 0
   THEN
      SELECT MACHINE_CODE, TRAIL_NO,LINE_NAME
        INTO C_MACHINECODE, C_TRAILNO,C_LINE_NAME
        FROM SMTINFO.R_SMT_PKGID_LOG_T
       WHERE PKG_ID = TRIM (PKG) AND (STATE_FLAG = 'N' OR STATE_FLAG = 'C') AND ROWNUM = 1;
       
       if  C_LINE_NAME =LINE THEN
 ...
 --  SFIS1.CHECK_PKG_MSD (EMP, PKG, LINE, PPN, 'N/A', RES);   DELETE BY ZC

  /* IF RES = 'OK'
   THEN
      INSERT INTO SFISM4.R_SMT_LOG_T
                  (STATION_NUMBER, MACHINE_CODE, PRODUCT_NO, VER, EMP_NO,
                   FEEDER_NO, KEY_PART_NO, WORK_TIME, SN, LINE_NAME, LOT_NO
                  )
           VALUES ('000', C_MACHINE, PPN, VER, EMP,
                   C_LOC, PKG, SYSDATE, 'N/A', LINE, LOC
                  );
      ...
      INSERT INTO SMTINFO.R_SMT_PKGID_LOG_T
                  (STATION_NUMBER, MACHINE_CODE, PRODUCT_NO, EMP_NO,
                   FEEDER_NO, TRAIL_NO, KEY_PART_NO, BEGIN_TIME, END_TIME,
                   LINE_NAME, PKG_ID, STATE_FLAG, FEEDER_STATE
                  )
           VALUES ('000', C_MACHINE, PPN, EMP,
                   C_LOC, C_LOC, C_KPN, SYSDATE, '',
                   LINE, PKG, 'N', ''
                  );

PROCEDURE        NEAT_ALLOCATE_MAC_SPU (V_INSN    IN     VARCHAR2,
                                           V_INMAC   IN     VARCHAR2,
                                            o_flag         OUT      VARCHAR2,  
                                           RES       OUT    VARCHAR2)
AS
   ...
   o_flag := '-1';
    ...
    IF INSTR(V_INMAC,'MAC;')>0 THEN
        ...
        IF INSTR(LEFTSTR,';',1,MACQTY-1)>0 THEN
            ...
            INSERT INTO SFISM4.R_MAC_T(SERIAL_NUMBER,TYPE,MODEL,QTY,MAC1,MAC2,MAC3,MAC4,MAC5,TEST_INFO,LASTEDITBY,LASTEDITDT)
            VALUES(V_INSN,TYPE_NAME,MODEL_NAME,MACQTY,MAC_1,MAC_2,MAC_3,MAC_4,MAC_5,V_INMAC,'TEST',SYSDATE);
        ELSE
            RES:=V_INSN||'\nMAC FORMAT ERROR!';
            RAISE E_NOSN;
        END IF; 

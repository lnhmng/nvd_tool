PROCEDURE        INVFBT (MACHINE_CODE     IN     VARCHAR2,
...
IS
   ...
   p_BARCODE           VARCHAR2 (25);
     P_ERRORDESC         VARCHAR2 (200);   --ADD BY LSC 20220302
    temp      VARCHAR2 (200);
    ...
    END IF; 
      ...
      */
      ...
      THEN
         IF INSTR (ERRORCODE, ';') > 0
         THEN
            if  SUBSTR (ERRORCODE, 1, 2) = '98' then       -- tzs 20240712 ????????,????ERRORCODE ????9842J5;na

               SELECT SUBSTR (ERRORCODE, 1, INSTR (ERRORCODE, ';') - 1)
                 INTO error_code_1
                 FROM DUAL;
                 p_ERRORCODE := error_code_1;  -- add tzs 20240712                 

            elsIF SUBSTR (ERRORCODE, 1, 1) = 'E'  
             -- IF SUBSTR (ERRORCODE, 1, 1) = 'E'
            THEN
               SELECT SUBSTR (ERRORCODE, 1, INSTR (ERRORCODE, ';') - 1)
                 INTO error_code_1

PROCEDURE                   check900inputs --Create by Steven Hu at 2008-03-01 for TAS-080301-01
                                                 (line      IN     VARCHAR2,
                                                  mygroup   IN     VARCHAR2,
                                                  DATA      IN     VARCHAR2,
                                                  res          OUT VARCHAR2)
...
BEGIN
   ...
   THEN
      ...
      ELSE
         ...
         RAISE e_null;
      END IF;
   ELSIF mygroup <> '900_INPUT' AND mygroup <> '900_INPUT_S'
   THEN
      res := 'OK';
      RAISE e_null;
   END IF;
---  add by 20201230 LY
  SELECT COUNT (serial_number)
   ...
   THEN
      ...
      IF substr(lock_station,0,9) ='900_INPUT' AND flag = 'LOCK'
      THEN
   ...
   THEN
      ...
      THEN
         ...
         THEN
            ...
            RAISE e_null;
         ELSE
            INSERT INTO sfism4.r_nvbios_model_spare_t (serial_number,
                                                       init_model_name,
                                                       first_bios,
                                                       second_bios,
                                                       last_model_name,
      ...
      ELSE
         ...
         ELSE
            ...
            ELSE
               ...
               SELECT a.serial_number, NVL (a.second_bios, a.first_bios)
                 INTO c_tempsn, v_bios
                 FROM sfism4.r_nvbios_model_t a, sfism4.r_sn_link_t b
               ...
               IF v_bios <> c_bios
               THEN                                --ADD BY tanrongliang begin
                  SELECT COUNT (*)
                    INTO c_count5
                  ...
                  IF c_count5 > 0
                  THEN
                     UPDATE sfism4.r_nvbios_comparelist_t
                        SET bios_nv = v_bios,
                            bios_900set = c_bios,
                            lp_900model = v_lp_900model,
                            station_name = mygroup,
                            comparetime = compare_date,
                            flag = 'N'
                      WHERE serial_number = DATA;
                  ...
                  --RAISE e_null;
                  ELSE
                     INSERT INTO sfism4.r_nvbios_comparelist_t
                          VALUES (DATA,
                                  v_bios,
                                  c_bios,
                                  v_lp_900model,
                     ...
                     SELECT MAX (RANK)
                       INTO V_RANK
                       FROM (SELECT ROW_NUMBER () OVER (ORDER BY comparetime)
                                       RANK
                               FROM sfism4.r_nvbios_comparelist_t a,
                                    (SELECT TO_CHAR (comparetime, 'YYYYMMDD')
                                               AS times,
                                            lp_900model AS lp
                                       FROM sfism4.r_nvbios_comparelist_t
                                      WHERE serial_number = DATA) b
                              WHERE     a.comparetime >
                                           TO_DATE (times,
                                                    'YYYYMMDDHH24MISS')
                                    AND a.lp_900model = b.lp
                                    AND a.flag = 'N');

                     UPDATE sfism4.r_nvbios_comparelist_t
                        SET ORDER_NUMBER = V_RANK
                      WHERE serial_number = DATA;
               ...
               ELSE
                  ...
                  ELSE
                     ...
                     COMMIT;
                  --RAISE e_null;
                  END IF;                            --ADD BY tanrongliang end
               END IF;

               UPDATE sfism4.r_nvbios_model_t
                  SET last_model_name = v_lp_900model, group_name = mygroup
                -- serial_number = DATA
                    ...
                    FROM (SELECT ROW_NUMBER () OVER (ORDER BY comparetime)
                            ...
                            FROM sfism4.r_nvbios_comparelist_t a,
                                 (SELECT TO_CHAR (comparetime, 'YYYYMMDD')
                                   ...
                                   WHERE serial_number = DATA) b
                           WHERE     a.comparetime >
                                        TO_DATE (times, 'YYYYMMDDHH24MISS')
                                 AND a.lp_900model = b.lp
                                 AND a.flag = 'N');
      ...
      ELSE
         ...
         THEN                                      --ADD BY tanrongliang begin
            ...
            ELSE
               ...
               SELECT MAX (RANK)
                 INTO V_RANK
                 FROM (SELECT ROW_NUMBER () OVER (ORDER BY comparetime) RANK
                         FROM sfism4.r_nvbios_comparelist_t a,
                              (SELECT TO_CHAR (comparetime, 'YYYYMMDD')
                                         AS times,
                                      lp_900model AS lp

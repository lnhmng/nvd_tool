        ...
        IF CUR1%NOTFOUND THEN
           ...
           ELSIF p_RESULT='F' THEN
              BEGIN
				...
				if  AOI_No='AOI12' OR AOI_No='AOI08' OR AOI_No='AOI10' or AOI_No='AOI21' or AOI_No='AOI14' or AOI_No='AOI16'
				    or AOI_No='AOI18' or AOI_No='AOI112' or AOI_No='AOI114' or AOI_No='AOI116' or AOI_No='AOI118' then
				   /******************* ????? ';'  ?????????**************/
				   if INSTR(ERROR_CODE,';')<=0 then
                	   INSERT INTO SFISM4.R_REPAIR_T(SERIAL_NUMBER,MO_NUMBER,TEST_TIME, TEST_CODE,TEST_STATION,TEST_LINE,RECORD_TYPE,MODEL_NAME)
                	   VALUES(p_SERIALNUMBER,p_MO,p_DATE,'AR013',p_STATION,p_LINE,'T',p_MODEL);
				...
				------For example: C118;C0603_B;14;null;null;null;
				if  AOI_No='AOI03' or AOI_No='AOI103' then
					/******************* ????? '?'  ?????????**************/
					if ((INSTR(ERROR_CODE,';'))=0)  then
                	    INSERT INTO SFISM4.R_REPAIR_T(SERIAL_NUMBER,MO_NUMBER,TEST_TIME, TEST_CODE,TEST_STATION,TEST_LINE,RECORD_TYPE,MODEL_NAME)
                	    VALUES(p_SERIALNUMBER,p_MO,p_DATE,'AR013',p_STATION,p_LINE,'T',p_MODEL);
				...
				-----(AOIIN) 3dx file format use semicolon to separate
				-----for example:13;60;0;
				IF AOI_NO='AOI05' OR AOI_NO='AOI07' OR AOI_NO='AOI09' OR AOI_NO='AOI22'
				   OR AOI_NO='AOI11' OR AOI_NO='AOI13' OR AOI_NO='AOI15' OR AOI_NO='AOI17' OR AOI_NO='AOI111'
				   OR AOI_NO='AOI113' OR AOI_NO='AOI115' OR AOI_NO='AOI117' OR AOI_NO='AOI19' THEN
				   /******************* ????? '?'  ?????????**************/
				   IF (INSTR(ERROR_CODE,';')<=0) THEN
				   	  INSERT INTO SFISM4.R_REPAIR_T(SERIAL_NUMBER,MO_NUMBER,TEST_TIME, TEST_CODE,TEST_STATION,TEST_LINE,RECORD_TYPE,MODEL_NAME)
                   	  VALUES(p_SERIALNUMBER,p_MO,p_DATE,'AR013',p_STATION,p_LINE,'T',p_MODEL);
				...
				-----(AOIOUT 3DX)
				-----for example:13;60;0;
				IF AOI_NO='AOI01' OR AOI_No='AOI02' OR AOI_NO='AOI20' THEN
				   /******************* ????? '?'  ?????????**************/
				   IF (INSTR(ERROR_CODE,';')<=0) THEN
				   	  INSERT INTO SFISM4.R_REPAIR_T(SERIAL_NUMBER,MO_NUMBER,TEST_TIME, TEST_CODE,TEST_STATION,TEST_LINE,RECORD_TYPE,MODEL_NAME)
                   	  VALUES(p_SERIALNUMBER,p_MO,p_DATE,'AR013',p_STATION,p_LINE,'T',p_MODEL);
